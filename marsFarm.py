...
while True:
    try:
        ...
        if auto:
            ...
            if float(phReading) > optimalRanges["ph"][
                    "top"] and currentTime - lastpHDose >= loopTime * 60:
                print("pH Down " + str(currentDay) + " " + str(currentTimeStr))
                phDown.write("D,2")
                lastpHDose = currentTime
            elif float(phReading) < optimalRanges["ph"][
                    "bottom"] and currentTime - lastpHDose >= loopTime * 60:
                print("pH Up " + str(currentDay) + " " + str(currentTimeStr))
                phUp.write("D,5")
                lastpHDose = currentTime
            if float(ecReading) > optimalRanges["ec"][
                    "top"] and currentTime - lastECDose >= loopTime * 60:
                print("Ec Down " + str(currentDay) + " " + str(currentTimeStr))
                waterOut.write("D,-5")
                sleep(5)
                waterIn.write("D,5")
                lastECDose = currentTime
            elif float(ecReading) < optimalRanges["ec"][
                    "bottom"] and currentTime - lastECDose >= loopTime * 60:
